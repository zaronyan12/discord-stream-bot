...
// ==============================================
// 修正: このファイル(webhookserver.js)にはDiscordクライアントや
// activeStreams、sendStreamNotification等が存在しないため、
// 以前の実装はここで直接通知しようとして必ずReferenceErrorになり、
// TwitCasting通知が一切送られない原因になっていた。
// YouTube Webhookと同様に、受け取った内容をそのままメインアプリ
// (index.js, ポート3001)へ転送するだけにする。
// イベント名の判定やペイロードのパース（実際は event:'livestart' で
// movie/broadcasterオブジェクトの中にネストされている）はメインアプリ側で行う。
  ...
  try {
    ...
    try {
      const response = await axios.post('https://10.138.0.4:3001/webhook/twitcasting', req.body, {
        timeout: 5000,
        httpsAgent: new https.Agent({
          rejectUnauthorized: false // 証明書検証無効(YouTube転送と同様、ローカル環境向けのため問題なし)
        })
      });
      console.log('メインアプリへのTwitCasting転送成功:', {
        status: response.status,
        responseData: response.data
      });
    } catch (axiosErr) {
      console.error('メインアプリへのTwitCasting転送エラー:', {
        message: axiosErr.message,
        response: axiosErr.response?.data,
        status: axiosErr.response?.status,
        stack: axiosErr.stack
