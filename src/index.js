...
client.on('interactionCreate', async interaction => {
 ...
 return interaction.reply({ content: 'このコマンドはボット製作者のみが使用できます。', ephemeral: true });
 }
 ...
 await interaction.reply({ content: `${user.tag} は既に管理者です。`, ephemeral: true });
 }
 ...
 if (!isAdmin) {
 return interaction.reply({ content: 'この操作は管理者のみが実行できます。', ephemeral: true });
 }
 ...
 const oauthUrls = {
 twitch_notification: `https://discord.com/api/oauth2/authorize?client_id=${encodeURIComponent(DISCORD_CLIENT_ID)}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20connections&state=twitch`,
 youtube_notification: `https://discord.com/api/oauth2/authorize?client_id=${encodeURIComponent(DISCORD_CLIENT_ID)}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20connections&state=youtube`,
 twitcasting_notification: `https://discord.com/api/oauth2/authorize?client_id=${encodeURIComponent(DISCORD_CLIENT_ID)}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=identify%20connections&state=twitcasting`,
 };

 if (oauthUrls[interaction.customId]) {
 await interaction.reply({
 content: `以下のURLで${interaction.customId.replace('_notification', '').toUpperCase()}アカウントをリンクしてください:\n${oauthUrls[interaction.customId]}`,
 ephemeral: true,
 });
 ...
 if (channel) {
 await channel.delete().catch(err => console.error(`チャンネル ${channel.id} の削除に失敗しました:`, err.message));
 }
 }
 }
 }
